var TextAnnotator=(function(){
    ...
    self.init=function(){
        ...
        var html="<div id='ADLmappings_annotatorDiv'style='width: 100%; height:300px;overflow: auto'></div>"
        $("#ADLmappings_dataSampleDiv").html(html)

        setTimeout(function(){

            var text="Over-range protection shall be provided for pressure instruments, pilots, gauges, etc. that may be subject to pressures that could damage or change the calibration of the instrument.\n" +
            ...
            $("#ADLmappings_annotatorDiv").html(text)
        $("#ADLmappings_annotatorDiv").bind("mouseup", function(event) {
    ...
    self.setAnnotation=function(ontologyNode){
        ...
        var color;
        if(ontologyNode.jstreeDiv=="ADLmappings_OneModelTree")
            color="lightgreen"

        else if(ontologyNode.jstreeDiv=="ADLmappingsjsOtherOntologiesTreeDiv")
            color="lightblue"
        else if(ontologyNode.jstreeDiv=="ADLmappingPropertiesTree")
            color="orange"
