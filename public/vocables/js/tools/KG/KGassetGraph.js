var KGassetGraph = (function () {
            ...
            [
                ...
                function (callbackSeries) {
                    ...
                    mappingsData.mappings.sort(function (a, b) {
                        ...
                        if ((subjectA = "id" && subjectB != "id")) return -1;
                        if (subjectA != "id" && subjectB == "id") return 1;
                        return 0;
                    });
                    ...
                    mappingsData.mappings.forEach(function (mapping) {
                        ...
                        if (true && subjectId.column == "id") {
                            ...
                            if (builtClasses[objectId]) shape = "ellipse";
                            ...
                            if (!predicates[mapping.predicate]) predicates[mapping.predicate] = {};
                            if (!predicates[mapping.predicate][mapping.subject]) predicates[mapping.predicate][mapping.subject] = [];
                        ...
                        if (objectObj) objectId = objectObj.table + "." + objectObj.column;
                        ...
                        if (!existingNodes[objectId] || objectId.indexOf("xsd") > -1) {
                            ...
                            if (mapping.predicate.indexOf("DatatypeProperty") > -1) color = "#dac";
                        ...
                        if (!existingNodes[edgeId]) {
                            ...
                            if (mapping.predicate != "http://www.w3.org/1999/02/22-rdf-syntax-ns#type") {
                                ...
                                if (modelObj) label = modelObj.label;
                                    ...
                                    if (p > -1) label = label.substring(p + 1);
                                }
                            }
                            visjsData.edges.push({
                                id: edgeId,
            ...
            function (err) {
                ...
                for (var predicate in predicates) {
                    ...
                    for (var subject in predicates[predicate]) {
                        if (!predicates2[predicate][classes[subject]]) predicates2[predicate][classes[subject]] = [];
    ...
    self.getBuiltMappingsStats = function (assetLabel, callback) {
        ...
        var query =
            ...
            "SELECT (COUNT(distinct ?sub) AS ?count) ?type " +
            fromStr +
            " WHERE {\n" +
            "  ?sub rdf:type ?type\n" +
            "} group by ?type";
        ...
        Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: assetLabel }, function (err, result) {
            ...
            result.results.bindings.forEach(function (item) {
                var color;
                if (KGbrowserCustom.superClassesMap[item.type.value]) color = KGbrowserCustom.superClassesMap[item.type.value].color;
                else color = Lineage_classes.getPropertyColor(item.type.value);
    ...
    self.drawClassesAndPropertiesGraph = function (source, graphDiv, options, callback) {
        if (!source) source = KGmappingData.currentSource;
        if (!source) {
        ...
        async.series(
                        ...
                        for (var predicate in result.predicates) {
                            for (var subject in result.predicates[predicate]) {
                                if (!self.buildClasses[subject]) {
                                    ...
                                    var color;
                                    if (KGbrowserCustom.superClassesMap[subject]) color = KGbrowserCustom.superClassesMap[subject].color;
                                    else color = Lineage_classes.getPropertyColor(subject);
                                ...
                                if (!self.classes[subject]) self.classes[subject] = {};
                                if (!self.classes[subject][predicate]) self.classes[subject][predicate] = [];
                                result.predicates[predicate][subject].forEach(function (object) {
                                    // if (self.buildClasses[object])
                                    if (object) self.classes[subject][predicate].push(object);
                                });
                            }
                        }
                        return callbackSeries();
                            ...
                            for (var predicate in self.classes[subject]) {
                                self.classes[subject][predicate].forEach(function (object) {
                                    ...
                                    if (!existingNodes[edgeId]) {
                                        ...
                                        if (self.model[predicate]) predicateLabel = self.model[predicate].label;
            ...
            function (err) {
                ...
                if (callback) return callback(null, { model: self.model, classes: self.classes });
            }
        );
    };
            ...
            [
                ...
                function (callbackSeries) {
                    ...
                    var query =
                        "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
                        "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n" +
                        "SELECT distinct ?subType ?prop ?objType ?subTypeLabel ?propLabel ?objTypeLabel  " +
                        fromStr +
                        "  WHERE {\n" +
                        "  ?sub ?prop ?obj.\n" +
                    ...
                    Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: sourceLabel }, function (err, result) {
                        ...
                        result.results.bindings.forEach(function (item) {
                            ...
                            if (!classes[item.subType.value]) classes[item.subType.value] = {};

                            if (!classes[item.subType.value][item.prop.value]) classes[item.subType.value][item.prop.value] = [];
                            if (classes[item.subType.value][item.prop.value].indexOf(item.objType.value) < 0) classes[item.subType.value][item.prop.value].push(item.objType.value);
                        });
                    ...
                    async.eachSeries(
                        ...
                        function (ids, callbackEach) {
                            ...
                            var query =
                                "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n" +
                                "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>\n" +
                                "SELECT distinct ?id ?idLabel where { ?id rdfs:label ?idLabel.  " +
                                filterStr +
                                " }";
                            ...
                            Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: sourceLabel }, function (err, result) {
                                ...
                                result.results.bindings.forEach(function (item) {
                                    var label;
                                    if (!item.idLabel) label = Sparql_common.getLabelFromURI(item.id.value);
                                    else label = item.idLabel.value;
