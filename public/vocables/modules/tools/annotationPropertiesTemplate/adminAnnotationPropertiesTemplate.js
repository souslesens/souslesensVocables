...
var AdminAnnotationPropertiesTemplate = (function () {
    ...
    self.renderAssignmentsTable = function (rows, options) {
        ...
        html += "<button class='btn btn-sm btn-outline-primary' onclick='AdminAnnotationPropertiesTemplate.openAssignmentsManager()'>Refresh</button>";
        html +=
        ...
        rows.forEach(function (row) {
            ...
            html += "<button ... onclick='AdminAnnotationPropertiesTemplate.viewRowTemplates(" + JSON.stringify(row.assignmentId) + ")'>View</button>";

            html += "<button class='btn btn-sm btn-outline-danger' onclick='AdminAnnotationPropertiesTemplate.deleteAssignment(" + JSON.stringify(row.assignmentId) + ")'>Delete</button>";
    ...
    function buildRowFromAssignment(assignmentListItem, assignmentContent, templatesInfoArray) {
        ...
        var targetId = assignmentContent && assignmentContent.targetId ? assignmentContent.targetId : assignmentListItem.data_source || "";

        // Aggregate templates
        var templateIds = (templatesInfoArray || []).map(function (t) {
            return t.id;
        });
        var templateLabels = (templatesInfoArray || []).map(function (t) {
            return t.label;
        });
