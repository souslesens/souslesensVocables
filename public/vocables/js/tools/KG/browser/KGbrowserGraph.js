var KGbrowserGraph = (function () {
    ...
    self.setGraphPopupMenus = function (node, _event) {
        ...
        var html =
            '<span class="popupMenuItem" style=\'font-weight: bold;color:' +
            KGbrowserQuery.model[node.data.type].color +
            "'> " +
            KGbrowserQuery.model[node.data.type].label +
            "</span>" +
            "    <span class=\"popupMenuItem\" style=' font - weight;:bold;'> " +
            node.label +
            "</span>" +
        ...
        html +=
            " expand :<br><select size='4' id='KGbrowser_graphMenuPredicateSelect' onchange='KGbrowserGraph.showExpandNodeFilterDiv($(this).val())'></select>" +
    ...
    self.setGraphPopupMenuAllowedExpandsSelect = function () {
      ...
      retainedPredicates.forEach(function(item) {
        ...
        var nodeId = self.currentGraphNode.id;
        var source = KGbrowser.currentSource;
        var fromStr = Sparql_common.getFromStr(source);
        var query = "select distinct * " + fromStr + " where {?s ?p ?o .?s rdf:type ?sType. ?o rdf:type ?oType. filter (?s=<" + nodeId + "> || ?o=<" + nodeId + ">)} limit 1000 ";
        ...
        Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: source }, function (err, result) {
            ...
            result.results.bindings.forEach(function (item) {
                if (!KGbrowserQuery.model[item.p.value] || !KGbrowserQuery.model[item.sType.value] || !KGbrowserQuery.model[item.oType.value]) return;
                var inverse = false;
                var objLabel = KGbrowserQuery.model[item.oType.value].label;
                if (item.p.value == nodeId) {
                    inverse = true;
                    objLabel = KGbrowserQuery.model[item.sType.value].label;
                }
                var uniquelabel = "" + (inverse ? "^" : "") + KGbrowserQuery.model[item.p.value].label + objLabel;
                if (!distinctPredicates[uniquelabel]) {
                    distinctPredicates[uniquelabel] = 1;

                    array.push({
                        id: item.p.value + "|" + item.oType.value + "|" + inverse,
                        label: uniquelabel,
                    });
                }
            });
    ...
    self.drawGraph = function (graphDiv, data, options, callback) {
        ...
        options.selectVars.forEach(function (varName) {
            ...
            var superClassGroup = null;
            if (KGbrowserCustom.superClassesMap[KGbrowserQuery.varNamesMap[varName].id]) superClassGroup = KGbrowserCustom.superClassesMap[KGbrowserQuery.varNamesMap[varName].id].group;
        ...
        data.data.forEach(function (item) {
            ...
            for (var key in keys) {
                ...
                var label = "";
                var type = "";
                if (item[key + "Label"]) label = item[key + "Label"].value;
                if (item[key + "Type"]) type = item[key + "Type"].value;
                ...
                KGbrowserQuery.queryFilterNodes.forEach(function (filter) {
                    if (filter.predicate && filter.predicate.object == type) {
                        ...
                        //transitive query(GraphTraversal)
                    }
    ...
    self.showExpandNodeFilterDiv = function (predicateStr) {
        ...
        Sparql_proxy.querySPARQL_GET_proxy(url, query, {}, { source: KGbrowser.currentSource }, function (err, result) {
            ...
            var html =
                "<button class='btn btn-sm my-1 py-0 btn-outline-primary' onclick='KGbrowserGraph.expandNode(\"all\")'>all</button> " +
    ...
    self.expandNode = function (selectedNodeId) {
        ...
        if (!selectedNodeId) selectedNodeId = $("#KGbrowserGraph_filterTargetNodesSelect").val();
        var data = [];
        for (var id in self.currentExpandingNode.targeNodes) {
            if (selectedNodeId == "all" || selectedNodeId == id) data.push(self.currentExpandingNode.targeNodes[id]);
        }
    ...
    self.expandGraph = function (predicateId) {
        ...
        nodes.forEach(function (item, index) {
            if (!existingClasses[item.data.type]) existingClasses[item.data.type] = [];
            existingClasses[item.data.type].push({ id: item.data.id, label: item.data.label });

            if (item.data.type == subject) ids.push(item.data.id);
            if (index > 0) idsStr += ",";
            idsStr += "<" + item.data.id + ">";
        });
        ...
        Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: source }, function (err, result) {
            ...
            result.results.bindings.forEach(function (item) {
                ...
                if (!existingNodes[id]) {
                    ...
                    if (!existingClasses[object]) existingClasses[object] = [];
