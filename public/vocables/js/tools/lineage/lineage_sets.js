var Lineage_sets = (function() {
    ...
    self.loadSetsJsTree = function() {
     var query =
        ...
        " select  distinct ?id ?label from <"+ Config.namedSetsThesaurusGraphUri+">"

      query+=  "   WHERE { " +
        " ?id skos:member <"+ Config.namedSetsThesaurusGraphUri+">. " +
        "?id skos:prefLabel ?label.}limit 10000";

      var sparql_url = Config.sources[self.tripleSetsThesaurusSourceLabel].sparql_server.url;
      if(sparql_url="_default")
        sparql_url=Config.default_sparql_url
...
// check if setName exists
          function(callbackSeries) {
            if (Config.sources[setName])
              alert(setName + "set already Exists");
