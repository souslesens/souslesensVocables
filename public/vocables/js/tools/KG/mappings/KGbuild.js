...
var KGbuild = (function () {
    ...
    self.initDialog = function () {
        ...
        if (!currentKG || currentKG == "") return alert("select an KG");
    ...
    self.buildTriples = function () {
        ...
        if (self.checked_tables.length == 0) return alert("Select mapped tables");
        ...
        if (replaceGraph && !confirm("erase existing graph")) return;
    ...
    self.serverMessage = function (message) {
        ...
        if (self.serverMessageCount % 100 == 0) $("#KGbuild_infosDiv").html("");
