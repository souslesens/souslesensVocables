...
class UserModel {
    ...
    findUserAccount = async (login) => {
        const conn = this._getConnection();
        const user = await conn.select("*").from("public_users_list").where("login", login).first();
        conn.destroy();
    ...
    findUserAccountFromToken = async (token) => {
        const conn = this._getConnection();
        const user = await conn.select("*").from("public_users_list").where("token", token).first();
        conn.destroy();
    ...
    checkUserPassword = async (login, password) => {
        const conn = this._getConnection();
        const user = await conn.select("password", "auth").from("users").where("login", login);
        conn.destroy();
        ...
        return true;  // This is not managed by the SLS authenticator
