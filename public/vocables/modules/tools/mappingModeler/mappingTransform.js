...
self._edgeToObjectMap = function (format, currentTable, fromNode, toNode, predicate, model) {
    ...
    if (isEntity) {
        ...
        var childData = resolveColumnData(fromData);
...
self._prefixBlock = function () {
    var baseOrder = ["rr", "rml", "ql", "csvw", "rdf", "rdfs", "xsd", "owl", ":"];
    var baseMap = {
    ...
    var extra = self._collectPrefixMap();
    for (var k in extra) {
        if (!map[k]) map[k] = extra[k];
    }

    var lines = [];
    baseOrder.forEach(function (p) {
        if (!map[p]) return;
        if (p === ":") lines.push("@prefix : <" + map[p] + "> .");
        else lines.push("@prefix " + p + ": <" + map[p] + "> .");
    });

    var extras = Object.keys(map).filter(function (p) {
        return baseOrder.indexOf(p) < 0;
    });
    extras.sort().forEach(function (p) {
