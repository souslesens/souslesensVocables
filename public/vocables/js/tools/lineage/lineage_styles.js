  ...
  self.listStyles = function(styleIds,callback) {
    Sparql_OWL.getAllTriples(self.stylesSourceLabel, null, styleIds, { source: self.stylesSourceLabel }, function(err, result) {
      ...
      result.forEach(function(item) {
        if (!objsMap[item.subject.value])
          objsMap[item.subject.value] = {};
        ...
        } else if (item.predicate.value == "http://souslesens.org/resource/styles/shape")
          objsMap[item.subject.value].shape = item.object.value;
        else if (item.predicate.value == "http://souslesens.org/resource/styles/size")
          objsMap[item.subject.value].size = item.object.value;
        else if (item.predicate.value == "http://souslesens.org/resource/styles/color")
          objsMap[item.subject.value].color = item.object.value;
        else if (item.predicate.value == "http://souslesens.org/resource/styles/icon")
          objsMap[item.subject.value].icon = item.object.value;

      });


if(callback)
  return callback(null,objsMap)
  ...
  self.displayStyle = function(styleUri) {
    ...
    if (!styleUri)
      self.currentSyleUri = $("#lineage_styles_listSelect").val();
    else
      self.currentSyleUri=styleUri;
  ...
  self.saveStyle = function() {
    ...
    if (!label)
      return alert("Enter a label for this style");
  ...
  self.linkStyle = function() {
    if (!self.currentClientObjectData)
      return alert("no client object");
    if (!self.currentSyleUri)
      return alert("no stylle selected");
