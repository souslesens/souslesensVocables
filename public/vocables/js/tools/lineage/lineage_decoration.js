...
var Lineage_decoration = (function() {
  ...
  self.fillcolorsMap = function() {
    var ids = [
      ...
      "http://rds.posccaesar.org/ontology/lis14/rdl/Aspect",
      "http://rds.posccaesar.org/ontology/lis14/rdl/Object"
    ];
        ...
        for (var i = 1; i <= 5; i++) {
          if (item["child" + i]) {
            if (!self.part14ColorsMap[item["child" + i].value]) {
              if (i == 1)
                self.part14ColorsMap[item["child" + i].value] = self.part14ColorsMap[item.concept.value];
              else
                self.part14ColorsMap[item["child" + i].value] = self.part14ColorsMap[item["child" + (i - 1)].value];
            }
          }
        }
      });
  ...
  self.listGraphNodeTypes = function(ids, part14TopTypes, callback) {
    ...
    async.eachSeries(
      ...
      function(slice, callbackEach) {
        ...
        if (part14TopTypes) {
          query += "  SELECT distinct ?x ?type ?g ?label" + strFrom + "WHERE {GRAPH ?g{" + "    ?x  rdf:type owl:Class. " +
            "OPTIONAL {?x rdfs:label ?label}" +
            " ?x   rdfs:subClassOf+ ?type.  filter(regex(str(?type),'lis14'))";

        } else {
          query += "SELECT distinct ?x ?type ?g ?label  " + strFrom + "  WHERE {GRAPH ?g{ ?x rdfs:subClassOf|rdf:type ?type." +
            "OPTIONAL {?x rdfs:label ?label}" +
    ...
    self.listGraphNodeTypes(nonPart14nodeIds, part14TopTypes, function(err, result) {
      ...
      if (self.usePart14Classes) {
        ...
        for (var _type in self.part14TopColorsMap) {
          ...
          str +=
            ...
            _type + "\")' style='background-color:" + self.part14TopColorsMap[_type] + "'>" +
      ...
      if (false) {
        ...
        for (var _type in nodesTypesMap) {
          str +=
            "<div class='Lineage_legendTypeDiv' onclick='Lineage_decoration.onlegendTypeDivClick($(this),\"" +
            _type + "\")' style='background-color:" + nodesTypesMap[_type].color + "'>" +
