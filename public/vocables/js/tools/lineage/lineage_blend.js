...
"  }" +*/
    ...
    self.graphModification = {
        ...
        addClassStiples: function () {
            ...
            if (!superClass) return alert("rdfs:label is mandatory");
                                    ...
                                    for (var prop in authorizedProps) {
                                        ...
                                        if (!distinctProps[propObj.prop]) {
                                            ...
                                            if (propObj.isGenericProperty) {
                                                ...
                                                label =
                                                    Sparql_common.getLabelFromURI(propObj.type.indexOf("D") ? sourceNodeFirstPartAncestor : "any") +
                                                    "-" +
                                                    propObj.label +
                                                    "->" +
            ...
            if (!props[item.prop.value]) {
              if (matchRange ) {
                ...
                if ((!item.domain && item.range) || (item.domain && item.domain.value == domain && (!item.range || item.range == range))) {
                    let type = "";
                    if (item.domain) type += "D";
                    if (item.range) type += "R";
