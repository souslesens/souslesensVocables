    ...
    ontologyUrl2tripleStore: function (url, sparqlServerUrl, graphUri, callback) {
        ...
        myParser.on("data", function (quad) {
          ...
          graphUri = quad.subejct.value;*/
            ...
            if (quad.predicate.value == "http://www.w3.org/2002/07/owl#imports") imports.push(quad.object.value);
        ...
        myParser.on("end", function (quad) {
            writer.end((error, triples) => {
                ...
                triplesArray.forEach(function (item, index) {
                    if (fetchCount++ < fechSize) tripleSlice += item + "\n";
                    else {
            ...
            [
                ...
                function (callbackSeries) {
                    ...
                    for (var source in sources) {
                        if (sources[source].graphUri == graphUri) sourceStatus.exists = source;
                        else sourceStatus.exists = false;
                    }
                    return callbackSeries();
                },
