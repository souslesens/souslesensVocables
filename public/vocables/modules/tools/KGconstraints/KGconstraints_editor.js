...
var KGconstraints_editor = (function () {
    ...
    self.getModelFromKG = function (callback) {
        var model = {}
        //getClassesFomKG
        async.series([function (callbackSeries) {

            OntologyModels.getAllClassesFromIndividuals(self.currentSource, null, function (err, result) {
                if (err) {
            ...
            OntologyModels.getClassPropertiesFromIndividuals(self.currentSource, Object.keys(model), null, function (err, result) {
                if (err) {
                    return callbackSeries(err)
                }
                ...
                result.forEach(function (item) {
                    ...
                    if (model[item.classId]) {
                        model[item.classId].properties[item.p] = item
                    }
                })
                callbackSeries()
            });
    ...
    self.startConstraintEidtorBot = function () {
        self.getModelFromKG(function (err, model) {
            if (err) {
                return alert(err)
            }
            ...
            KGconstraintsEditor_bot.start(KGconstraintsEditor_bot.workflow_dataTypePropertyConstraint, params, function (err, result) {
                ...
                self.writeConstraint(result)

            })
        })
    }

    self.writeConstraint = function () {
        var data = KGconstraintsEditor_bot.params
    }
