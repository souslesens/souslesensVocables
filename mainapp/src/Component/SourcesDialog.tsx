...
export const SourcesDialog = ({ edit, me, onClose, onSubmit, open, selectedSource, sources }: SourcesDialogProps) => {
    ...
    const handleValidation = (data: ServerSource) => {
        const schema = ServerSourceSchema.superRefine(
            (value: ServerSource, context) => {
            ...
            <DialogContent>
                <Stack spacing={2} sx={{ pt: 1, maxHeight: "75%" }}>
                    ...
                    <Autocomplete
                        ...
                        options={predicates}
                        renderInput={(params) => <TextField error={errors.taxonomyPredicates !== undefined ? errors.taxonomyPredicates : undefined} helperText={errors.taxonomyPredicates} {...params} label="Taxonomy Predicates" />}
                                ...
                                <Stack spacing={2} useFlexGap>
                                    ...
                                    <FormGroup>
                                        {Object.entries(permissionLabels).map(([key, label]: string[]) => (
                                            <FormControlLabel
                                                control={<Checkbox checked={source[key]} key={`check-${key}`} onChange={(event) => handleField(key, event.target.checked)} />}
                                                key={key}
                                                label={label}
                                            />
                                        ))}
