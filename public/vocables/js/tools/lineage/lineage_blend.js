...
var Lineage_blend = (function() {
 ...
 $("#GenericTools_searchInAllSources").prop("checked", true)*/
  ...
  self.createRelation = function(inSource,type, sourceNode, targetNode, addImportToCurrentSource, createInverseRelation, callback) {
  ...
  self.graphModification = {
      ...
      $("#LineagePopup").load("snippets/lineage/lineageAddEdgeDialog.html", function() {
        ...
        let options = {
          ...
          selectTreeNodeFn: function(event, obj) {
            ...
            Lineage_blend.graphModification.createRelationFromGraph(obj.node.data.inSource,self.sourceNode, self.targetNode, obj.node.data.id, function(err, result) {
              if (err)
                return callback(err);
        ...
        async.series([
            function(callbackSeries) {
              ...
              jstreedata2.push({
                ...
                text: "owl:sameAs",
                parent: "#",
                data:{id: "http://www.w3.org/2002/07/owl#sameAs",inSource:Config.dictionarySource}
              });
              // var data = ["owl:sameAs"]
    ...
    createRelationFromGraph: function(inSource,sourceNode, targetNode, propId, callback) {

      if(!confirm( "create Relation "+sourceNode.label+"-"+Sparql_common.getLabelFromURI(propId)+"->"+targetNode.label))
        return;
