var Lineage_dictionary = (function() {
  ...
  self.dataTablesOrderedColumns = ["status", "domainSourceLabel", "domainLabel", "rangeSourceLabel", "rangeLabel"];
    ...
    async.series(
      [
        ...
        function(callbackSeries) {
          var dataset = [];
          var cols = [];
          cols.push({ title: "restrictionNode", defaultContent: "" });
  ...
  self.validation = {
    ...
    promote: function(data) {
      ...
      self.validation.executeRestrictionSparql(data, "promote");
    }
    ,
    unPromote: function(data) {
      self.validation.executeRestrictionSparql(data, "unPromote");
    }
    , remove: function(data) {
      self.validation.executeRestrictionSparql(data, "remove");
    },

    executeRestrictionSparql: function(data, operation) {
      if (data.length == 0)
        return alert("no row is selected");
      if (!confirm(operation + " " + data.length + " dictionary entries ?"))
        return;
      var restrictionIds = [];
      ...
      async.earchSeries(slices, function(restrictionIds, callbackEach) {
        ...
        if (operation == "promote") {
          ...
          query += " insert { ?node <" + Config.dictionaryMetaDataPropertiesMap["status"] + "> 'OK'} ";
          query += " where { ?node <" + Config.dictionaryMetaDataPropertiesMap["status"] + "> ?status " + filter + "} ";
        } else if (operation == "unPromote") {
          query += "delete{ ?node <" + Config.dictionaryMetaDataPropertiesMap["status"] + "> ?status} ";
          query += " insert { ?node <" + Config.dictionaryMetaDataPropertiesMap["status"] + "> 'Candidate'} ";
          query += " where { ?node <" + Config.dictionaryMetaDataPropertiesMap["status"] + "> ?status " + filter + "} ";
        }
        if (operation == "remove") {
