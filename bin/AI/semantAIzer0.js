...
var SYSTEM_PROMPT = `
Des listes te sont transmises en base64 (JSON encodé UTF-8).
Décode-les, puis retourne UNIQUEMENT un CSV avec les colonnes :
"id_a,label_a,id_b,label_b,score"
assemble les  listA et listB en te basant sur le champ label de chacune
la rappochement doit se faire sur l'ensemble des tokens (tokenized)) 
distance de levenstein de chaque token doit être >0.6 pour les label composés de plusieurs token 
    ...
    async function run() {
        ...
        for (let i = 0; i < chunks.length; i += CONCURRENCY) {
            ...
            try {
                ...
                // ajout CF to keep only best matches

                var allMatchesMap = {};

                allMatches.forEach(function (item) {
                    allMatchesMap[item.a] = item;
                });
                results.forEach(function (result2) {
                    result2.forEach(function (item) {
                        if (!allMatchesMap[item.a] || allMatchesMap[item.a].score < item.score) {
