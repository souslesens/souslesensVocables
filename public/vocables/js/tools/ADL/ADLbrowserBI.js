var ADLbrowserBI = (function () {
    ...
    self.drawClasses = function () {
        ...
        var type = ADLbrowser.currentJstreeNode.id
        ...
        if (continousProp) {
            ...
            var query = " PREFIX  rdfs:<http://www.w3.org/2000/01/rdf-schema#> PREFIX  rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX owl:<http://www.w3.org/2002/07/owl#> Select  distinct ?sub ?obj  " +
                fromStr +
                "where { ?sub <" + property + "> ?obj . ?sub rdf:type <" + type + ">. " +
                filterStr +
                "} limit 10000"
            ...
            Sparql_proxy.querySPARQL_GET_proxy(url, query, {}, {source: ADLbrowser.currentSource}, function (err, result) {
                ...
                result.results.bindings.forEach(function (item) {
                   ...
                   else if (item.datatype && item.datatype == "http://www.w3.org/2001/XMLSchema#float")
                        data.push({id: item.sub.value, val: parseFloat(item.obj.value)})
                    else if (item.datatype && item.datatype == "http://www.w3.org/2001/XMLSchema#integer")
                        data.push({id: item.sub.value, val: parseInt(item.obj.value)})
                    else
                        return;
