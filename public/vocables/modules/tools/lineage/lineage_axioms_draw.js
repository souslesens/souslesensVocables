  ...
  self.drawNodeAxioms = function(sourceLabel, nodeId, divId, depth, options, callback) {
    ...
    async.series(
      [
        ...
        // set source for each node
        function(callbackSeries) {
          var ids = Object.keys(allBasicAxioms);

          Sparql_OWL.getUrisNamedGraph(sourceLabel, ids, null, function(err, result) {

            result.forEach(function(item) {
              var realSource = Sparql_common.getSourceFromGraphUri(item.g.value, self.currentSource);
              allBasicAxioms[item.id.value].source = realSource;
            });
            return callbackSeries();
          });
      ...
      if(existingNodes[object.o]<level)// dont draw backward edges
        return
