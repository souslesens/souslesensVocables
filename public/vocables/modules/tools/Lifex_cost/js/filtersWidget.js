...
var FiltersWidget = (function () {
    ...
    self.loadTree = function (source) {
        ...
        JstreeWidget.loadJsTree(self.jstreeDiv, jstreeData, options, function (err) {
            ...
            $("#" + self.jstreeDiv).jstree().check_node('withoutJCNeededPost2029');
            $("#" + self.jstreeDiv).on("hover_node.jstree", function (e, data) {
                ...
                var jstreeNode = $("#" + self.jstreeDiv).jstree().get_node(htmlNode);
    ...
    self.onSelectedNodeTreeclick = function (event, obj, callback) {
        ...
        if (self.currentNode.id == "Activity") {
            ...
            if ($("#" + self.jstreeDiv).jstree().get_node("JobCard").children_d.length > 0) {
                var descendants = $("#" + self.jstreeDiv).jstree().get_node("JobCard").children_d;
            ...
            if ($("#" + self.jstreeDiv).jstree().get_node("Tag").children_d.length > 0) {
                var descendants = $("#" + self.jstreeDiv).jstree().get_node("Tag").children_d;
                $("#" + self.jstreeDiv).jstree().uncheck_node(descendants);
            ...
            if ($("#" + self.jstreeDiv).jstree().get_node(self.currentNode.id).children_d.length > 0) {
                var descendants = $("#" + self.jstreeDiv).jstree().get_node(self.currentNode.id).children_d;
                descendants.forEach(function (node) {
                    $("#" + self.jstreeDiv).jstree().delete_node(node);
                });
                //$("#" + self.jstreeDiv).jstree().uncheck_node(descendants);
    ...
    self.getQueryFiltersFromUI = function (groupByKey, filter, callback) {
        ...
        if (groupByKey == "Task Keyword") {
            var children = $("#" + self.jstreeDiv).jstree().get_node("TaskKeyword").children;
        ...
        for (var type in types) {
            ...
            if (type == "Discipline") {
                ...
                subquery = " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/\n" +
                ...
                subquery = "?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/<http://rds.posccaesar.org/ontology/lis14/rdl/residesIn> ?functionalLocation.\n" +
                    "?functionalLocation  rdf:type <http://data.total/resource/tsf/dalia-lifex1/FunctionalLocation>." +
                    "?functionalLocation <http://www.w3.org/2000/01/rdf-schema#label> ?functionalLocationLabel.";
                ...
                subquery = "?JobCardExecution ^<http://rds.posccaesar.org/ontology/lis14/rdl/activityPartOf> ?Task. " +
                ...
                subquery = " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant> ?tag.\n"
                    + "?tag <http://www.w3.org/2000/01/rdf-schema#label> ?tagLabel." +
                    "?tag <http://www.w3.org/2000/01/rdf-schema#label> ?tagLabel." + "OPTIONAL  {?tag <http://purl.org/dc/terms/title> ?tag_title.}";
                ...
                subquery += " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/<http://rds.posccaesar.org/ontology/lis14/rdl/residesIn> / <http://data.total/resource/tsf/dalia-lifex1/unitName> ?unitName. ";
                subquery += Sparql_common.setFilter("unitName", null, types[type].labels).replace("Label", "");
                subQueries.push(subquery);

            } else if (type == "Sector") {

                subquery += " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/<http://rds.posccaesar.org/ontology/lis14/rdl/residesIn> / <http://data.total/resource/tsf/dalia-lifex1/sectorName> ?sectorName. ";
                subquery += Sparql_common.setFilter("sectorName", null, types[type].labels).replace("Label", "");
                subQueries.push(subquery);
            } else if (type == "System") {
                subquery += " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/<http://rds.posccaesar.org/ontology/lis14/rdl/residesIn> / <http://data.total/resource/tsf/dalia-lifex1/systemName> ?systemName. ";
        ...
        if (groupByKey == "FLOC" && !types["FLOC"]) {
            subquery += " ?JobCardExecution <http://rds.posccaesar.org/ontology/lis14/rdl/hasPassiveParticipant>/<http://rds.posccaesar.org/ontology/lis14/rdl/residesIn> / <http://www.w3.org/2000/01/rdf-schema#label> ?functionalLocationLabel. ";
            subQueries.push(subquery);
