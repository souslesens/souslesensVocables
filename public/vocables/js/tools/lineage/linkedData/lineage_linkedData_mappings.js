  ...
  var Lineage_linkedData_mappings = (function() {
    ...
    self.createGraphRelationJoin = function() {
      ...
      if (!self.currentDatabase)
        return alert("select a database");
      ...
      self.queryColumnsMappings(Lineage_sources.activeSource, {
        filter: filterStr,
    ...
    self.writeJoinMapping = function() {
      ...
      if (!fromColumn || !toColumn)
        return alert ("enter fromColumn and toColumn ");
   ...
   //   var sql = "select * from " + relation.fromColumn.table + "," + relation.toColumn.table + " " + joinClause;
    ...
    self.queryColumnsMappings = function(source, options, callback) {
      ...
      if (options.database)
        query += "?column slsv:database ?database.filter (?database='" + options.database + "') ";
