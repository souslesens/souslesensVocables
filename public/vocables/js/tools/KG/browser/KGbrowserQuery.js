var KGbrowserQuery = (function () {
    ...
    self.executeShortestPathQuery = function (fromNode, toNode, options, callback) {
        ...
        if (self.queryFilterNodes.length < 2) return alert("this query needs at least 2 superClasses ");
    ...
    self.initClassesTab = function (classes, model) {
        ...
        classesArray.forEach(function (classId) {
            ...
            if (!distinctNode[id1]) {
                distinctNode[id1] = 1;
                var jstreeType = KGbrowserCustom.superClassesMap[classId].group;
                jstreeData.push({
                    id: id1,
                    text: label,
                    parent: "#",
            ...
            for (var predicate in classes[classId]) {
                if (!model[predicate]) var x = predicate;
                    ...
                    classes[classId][predicate].forEach(function (object) {
                        ...
                        if (!distinctNode[id3]) {
                            distinctNode[id3] = 1;
                            var jstreeType = KGbrowserCustom.superClassesMap[object].group;
                            jstreeData.push({
                                id: id3,
