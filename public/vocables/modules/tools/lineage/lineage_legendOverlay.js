...
var Lineage_legendOverlay = (function () {
  ...
  self.getNodeCategory = function (node) {
    ...
    if (
      (node.shape === "box" || node.shape === "square") &&
      node.data &&
      node.id === node.data.source
    ) {
      return "Source";
    }
    ...
    if (
      ...
      node.data.label &&
      node.data.source &&
      LegendOverlayWidget.normalizeColor(LegendOverlayWidget.getColor(node, "")) ===
        LegendOverlayWidget.normalizeColor("#ddd")
    ) {
      return "Property";
    }
  ...
  self.getEdgeCategory = function (edge, nodesByIdMap) {
    ...
    if (edge.dashes) {
      ...
      var datatypeNorm = LegendOverlayWidget.normalizeColor(
        self.legendOptions && self.legendOptions.datatypeColor
      );

      var lbl = edge.label ? String(edge.label) : "";
      var looksLikeDatatype =
        lbl.indexOf("xsd:") === 0 || lbl.indexOf("http://www.w3.org/2001/XMLSchema#") === 0;
    ...
    if (nodesByIdMap && edge.dashes) {
      ...
      var inverseOfNorm = LegendOverlayWidget.normalizeColor(
        (self.legendOptions && self.legendOptions.inverseOfColor) || "#0067bb"
      );
    ...
    if (isDashed) {
      ...
      var restrictionNorm = LegendOverlayWidget.normalizeColor(
        self.legendOptions && self.legendOptions.restrictionColor
      );
      var datatypeNorm2 = LegendOverlayWidget.normalizeColor(
        self.legendOptions && self.legendOptions.datatypeColor
      );
    ...
    if (nodesByIdMap && !edge.label && !edge.dashes) {
      ...
      var rdfsMemberNorm = LegendOverlayWidget.normalizeColor(
        (self.legendOptions && self.legendOptions.rdfsMemberColor) || "#e7a1be"
      );
