...
var codeRegex501 = {
    ...
    "xxxx":/^(?<sector>[\w\d]{1,2})-*(?<item>\w{1,3})-(?<system>\d)(?<subSystem>\d)(?<seqNum>\d{1,2})-*(?<suffix>[\w\d]{0,5})-*.*$/g,//6-ELM-8310-18H
...
var decodeFunctionalLocation = function () {
    ...
    var sql = "   SELECT [dbo].[tblTagFormat_UK].name format,[dbo].[tblTagFormatGroup_UK].name positionMeaning,[dbo].[tblTagFormatGroup_UK].[order] ,[dbo].[tblTagFormatPermittedValue_UK].[PermittedValue],[Meaning] valueMeaning\n" +
        "\n" +
    ...
    sqlServer.getData(dbName, sql, function (err, result2) {
        ...
        result2.forEach(function (item) {
            ...
            if (!formatMap[item.format])
                formatMap[item.format] = {}
            if (!formatMap[item.format] [item.order])
                formatMap[item.format] [item.order] = {}
