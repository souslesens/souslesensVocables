...
var NodeInfosAxioms = (function() {
    ...
    self.init = function(source, resource) {
        ...
        $("#nodeInfosWidget_AxiomsTabDiv").load("modules/tools/axioms/html/nodeInfosAxioms.html", function() {
            ...
            Axiom_editor.initResourcesMap(self.currentSource, function(err, result) {// used do draw graph
                self.initSourceClassesMap(self.currentSource, function(err, result) {//used to parse manchester
                    if (err) {
                        return alert(err);
                    }
        ...
        self.getResourceAxioms({}, function(err, result) {
            ...
            manchester.forEach(function(item, index0) {
                ...
                if (item.indexOf("DisjointClasses") == 0) {
                    ...
                    item.substring(item.indexOf(":") + 1).split(",").forEach(function(superClass) {
                        jstreeData.push({
                            id: "superClass",
                            text: "superClass",
