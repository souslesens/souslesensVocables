...
var Lineage_axioms_draw = (function () {
    ...
    self.drawNodeAxioms = function (sourceLabel, nodeId, divId, depth, options) {
        ...
        self.getNodeAxioms(sourceLabel, nodeId, depth, options, function (err, result) {
            ...
            var geNodeParams = function (type) {
            ...
            function recurse(nodeId, level) {
                ...
                if (!existingNodes[item.s.value]) {
                    var nodeParams = geNodeParams(item.sType ? item.sType.value : null);
                ...
                item.children.forEach(function (child) {
                    ...
                    if (!targetItem) {
                        ...
                        if (!existingNodes[targetItem.s.value]) {
                            var nodeParams = geNodeParams(targetItem.sType ? targetItem.sType.value : null);
                            var options = nodeParams.options;
                            edgeStyle = nodeParams.edgeStyle;
                            ...
                            result.forEach(function (item) {
                                ...
                                visjsData.nodes.forEach(function (item, index) {
                                    if (finalNodes[item.data.id]) {
                                        var options = geNodeParams(finalNodes[item.data.id].type).options;
