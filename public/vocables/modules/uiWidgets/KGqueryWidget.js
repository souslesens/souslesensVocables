...
html += "<button class='KGqueryWidget_divActions btn btn-sm my-1 py-0 btn-outline-primary' about='add filter' onclick='KGqueryWidget.groupBy(\"" + nodeDivId + "\");'>⋃</button>";
  ...
  self.execPathQuery = function(currentQuery, options, callback) {
    ...
    if (self.combinedQueries.length == 0) {
      self.combinedQueries.push({ query: self.currentQuery });
    }
    var predicateStr = "";
    var filterStr = "";
    var optionalStr = "";
    ...
    var whereStr="";
    self.combinedQueries.forEach(function(combinedQuery, combinedQueryIndex) {
