...
KGbrowserCustom.initsuperClassesPalette()
        ...
        setTimeout(function () {
         ...
         //   $("#KGbrowserItemsjsTreeDiv").css("height","400px !important")
    ...
    self.query = {
        ...
        showQueryParamsDialog: function (position) {
            if (false && KGbrowser.currentGraphNodeSelection) {
                ...
                var possibleTypes = KGbrowser.query.getAdlModel(KGbrowser.currentGraphNodeSelection.data.type, KGbrowser.currentGraphNodeSelection.data.source, "subjectOrObject", function (err, result) {
                    if (err)
                        return alert(err)
        ...
        executeSparqlQuery: function (showQueryOnly, callback) {
            ...
            allNodes.forEach(function (item) {
                console.log(item.id)
                if ($("#KGbrowser_queryTreeDiv").jstree(true).is_checked(item.id))
                    item.inResult = true
                ...
                if (node.data.type == "type") {//type
                    ...
                    if (index > 0) {//relation anonyme avec le precedent type
                        if (node.data.sourceType == "rdl") {
                            if (sources.indexOf(Config.KG.RDLsource) < 0)
                                sources.push(Config.KG.RDLsource)
