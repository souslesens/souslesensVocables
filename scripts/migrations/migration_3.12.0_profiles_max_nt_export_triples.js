...
const migrateProfiles = async (configDirectory, writeMode) => {
    ...
    if (!(await connection.schema.hasColumn("profiles", "max_nt_export_triples"))) {
        if (writeMode) {
            await connection.schema.alterTable("profiles", function (table) {
                table.integer("max_nt_export_triples");
            });
...
const migrateProfilesList = async (configDirectory, writeMode) => {
    ...
    if (!(await connection.schema.hasColumn("profiles_list", "max_nt_export_triples"))) {
        if (writeMode) {
