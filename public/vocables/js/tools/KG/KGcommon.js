                        ...
                        OwlSchema.initSourceSchema(self.currentSource, function (err, schema) {
                            ...
                            Sparql_schema.getPropertiesRangeAndDomain(schema, null, null, null, function (err, result) {
                                ...
                                result.forEach(function (item) {
                                    if (item.subProperty) {
                                        if (!subPropertiesMap[item.property.value]) subPropertiesMap[item.property.value] = [];
                                        ...
                                        self.allObjectsMap[item.subProperty.value] = {
                                            ...
                                            data: { label: item.propertyLabel.value, id: item.subProperty.value },
                                        };
                                    }
                                });
                                ...
                                result.forEach(function (item) {
                                    ...
                                    if (item.range || item.domain) {
                                        self.allObjectsMap[item.property.value] = {
                                            ...
                                            data: { label: item.propertyLabel.value, id: propertyId },
                                        };

                                        if (item.domain) {
                                            var domainId = item.domain.value;
                                            if (!self.constraintsMap.domains[domainId]) self.constraintsMap.domains[domainId] = [];
                                            if (self.constraintsMap.domains[domainId].indexOf(propertyId) < 0) self.constraintsMap.domains[domainId].push(propertyId);
                                        }
                                        if (item.range) {
                                            var rangeId = item.range.value;
                                            if (!self.constraintsMap.ranges[rangeId]) self.constraintsMap.ranges[rangeId] = [];
                                            if (self.constraintsMap.ranges[rangeId].indexOf(propertyId) < 0) self.constraintsMap.ranges[rangeId].push(propertyId);
                                        }
                                    }
                                });
                    ...
                    function (callbackSeries) {
                        ...
                        Sparql_OWL.getObjectRestrictions(self.currentSource, null, null, function (err, result) {
                            ...
                            objs.forEach(function (obj) {
                                if (!self.constraintsMap.domains[obj.domain]) self.constraintsMap.domains[obj.domain] = [];
                                if (self.constraintsMap.domains[obj.domain].indexOf(obj.property) < 0) self.constraintsMap.domains[obj.domain].push(obj.property);

                                if (!self.constraintsMap.ranges[obj.range]) self.constraintsMap.ranges[obj.range] = [];
                                if (self.constraintsMap.ranges[obj.range].indexOf(obj.property) < 0) self.constraintsMap.ranges[obj.range].push(obj.property);

                                if (!self.constraintsMap.properties[obj.property]) self.constraintsMap.properties[obj.property] = {};

                                if (!self.constraintsMap.properties[obj.property][obj.domain]) self.constraintsMap.properties[obj.property][obj.domain] = {};
                                ...
                                if (subPropertiesMap[obj.property]) {
                                    subPropertiesMap[obj.property].forEach(function (subProperty) {
                                        if (!self.constraintsMap.properties[subProperty.id]) self.constraintsMap.properties[subProperty.id] = {};

                                        if (!self.constraintsMap.properties[subProperty.id][obj.domain]) self.constraintsMap.properties[subProperty.id][obj.domain] = {};
