...
const UserDataObject = z
    .object({
        ...
        data_content: z.record(z.string(), z.any()).optional(),
...
class UserDataModel {
    ...
    insert = async (userData) => {
        ...
        const connection = getKnexConnection(this._mainConfig.database);
        const results = await connection.select("id", "login").from("users").where("login", data.owned_by).first();
        if (results === undefined) {
            cleanupConnection(connection);
        ...
        const fileName = `${identifier}-${results.login}-${ULID.ulid()}.json`;
    ...
    remove = async (identifier) => {
        ...
        const connection = getKnexConnection(this._mainConfig.database);
        const results = await connection.select("id", "data_path").from("user_data").where("id", identifier).first();
        if (results === undefined) {
            cleanupConnection(connection);
            throw Error("The specified identifier do not exists", { cause: 404 });
        }
    ...
    update = async (userData) => {
        ...
        const user = await connection.select("*").from("users").where("login", data.owned_by).first();
        if (user === undefined) {
            cleanupConnection(connection);
            throw Error("The specified owned_by do not exists", { cause: 404 });
        }
