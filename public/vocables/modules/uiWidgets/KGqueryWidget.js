...
var KGqueryWidget = (function () {
    ...
    self.execPathQuery = function (allQueryPathes, options, callback) {
        ...
        queryPath.path.forEach(function(pathItem, pathIndex) {
          // if (pathIndex > 0) {
          //  predicateStr += "/";
          var transitionClassId = (pathItem.length == 4) ? pathItem[1] : pathItem[0];
          var transitionVarName = self.getVarName({ id: transitionClassId });
          //   var transitionPredicate=subjectVarName + "  "+ "<" + pathItem[2] + ">"+" "+transitionVarName+"."
          if (!directPredicatesArray[pathItem.toString()]) {
            //each transition predicate has to be unique
            directPredicatesArray[pathItem.toString()] = 1;
            predicateStr += " " + transitionVarName + ". " + transitionVarName + "  ";
