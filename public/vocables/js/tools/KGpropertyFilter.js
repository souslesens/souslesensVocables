var KGpropertyFilter = (function() {
  ...
  self.showAssociateDialog = function() {
    ...
    for (var key in self.aspectsMap)
      var items = $("#" + selectId).jstree().get_checked(true);
  ...
  self.associateFiltersToPropertyRestriction = function() {
    ...
    leftObjs.forEach(function(item) {
      if (item.data.type!="Property")// filter selected Properties Only
        return;
      selectedProperties.push(item.data);
    });
    if (selectedProperties.length == 0)
      return alert("no property is selected");


    var aspectObjs = $("#" + self.aspectsMap[self.currentAspect].treeDiv).jstree().get_checked(true);
    if (aspectObjs.length == 0)
      return alert("no aspect value  is selected");

    if (!confirm("Associate " + selectedProperties.length + "properties to  aspect selected  values of aspect" + self.currentAspect))
      return;
