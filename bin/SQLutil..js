...
var SQLutil = {
    ...
    csvToJson: function (csvFilePath, sliceLength, callback) {
        ...
        if (!fs.existsSync(csvFilePath))
            return callback("file does not exists :" + csvFilePath)
    ...
    createTableFromCsv: function (connection, dbName, tableName, csvFilePath) {
        SQLutil.csvToJson(csvFilePath, 1000, function (err, result) {
            ...
            result.data[0].forEach(function (item, index) {
                ...
                result.headers.forEach(function (header,indexHeader) {
                    ...
                    if (util.isInt(value)) {
                        ...
                        type = "varchar"
                        if(value &&  value != "")
                        insertStr+="'"+value.replace(/'/g,"")+"'"
                    }
                    if (!columns[header])
                        columns[header] = {type: type, length: 0}
          ...
          var insertStr="INSERT into "+tableName+" VALUES \n"+insertStr+"\n"
            var str=""
          var  dropTable=true
            if( dropTable)
