      ...
      $("#LineagePopup").load("snippets/lineage/lineageAddEdgeDialog.html", function() {
        ...
        async.series(
          [
            ...
            function(callbackSeries) {
              if (self.currentSpecificObjectPropertiesMap)
                return callbackSeries();
              ...
              Sparql_OWL.listObjectProperties(specificSourceLabel, null, function(err, result) {
                ...
                result.forEach(function(item) {
                  if (item.superProp) {
                    if (!self.currentSpecificObjectPropertiesMap[item.superProp.value])
                      self.currentSpecificObjectPropertiesMap[item.superProp.value] = [];
            ...
            function(callbackSeries) {
              options.contextMenu = {
                refineProperty: {
                  ...
                  action: function(_e) {
                    if (self.currentPropertiesTreeNode.data.source != Config.formalOntologySourceLabel)
                      return alert("only properties form lis14 can be refined");
