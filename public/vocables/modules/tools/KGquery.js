...
import TimeLineWidget from "../uiWidgets/timeLineWidget.js";
...
var KGquery=(function(){
  ...
  self.onLoaded=function(){
     $("#actionDivContolPanelDiv").load("snippets/KGquery/KGquery_leftPanel.html", function () {

     })
       $("#graphDiv").load("snippets/KGquery/KGquery_centralPanel.html", function () {
  ...
  self.showSourcesDialog = function (forceDialog) {
    if (!forceDialog && Config.tools["lineage"].urlParam_source) {
     self.currentSource=urlParam_source;
     self.drawVisjsModel()
    }
...
return alert("missing target node in  path");
    ...
    self.saveModelGraph = function () {
      ...
      var payload = {
        dir: "graphs/",
        fileName: self.currentSource + "_KGmodelGraph.json",
        data: str,
      };
    ...
    self.drawVisjsModel = function () {
      var visjsData;
      var visjsOptions = {
        onclickFn: KGquery.graphActions.onNodeClick,
        dndCtrlFn: KGquery.graphActions.onDnDnode,
      };
      ...
      function draw() {
        ...
        self.KGqueryGraph = new VisjsGraphClass("KGquery_graphDiv", visjsData, visjsOptions);
      ...
      var visjsGraphFileName = self.currentSource + "_KGmodelGraph.json";

      self.KGqueryGraph = new VisjsGraphClass("KGquery_graphDiv", { nodes: [], edges: [] }, visjsOptions);
      ...
      if (true) {
        self.KGqueryGraph.loadGraph(visjsGraphFileName, null, function(err, result) {
          ...
          //  return draw();
          self.getInferredModelVisjsData(self.currentSource, function(err, result2) {
        ...
        self.getInferredModelVisjsData(self.currentSource, function(err, result) {
          if (err) {
            return alert(err.responseText);
          }
          visjsData = result;
