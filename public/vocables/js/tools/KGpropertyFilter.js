  ...
  { "width": "50px", "targets": 4 },
    ...
    self.matrix = {
        getMatrixHtml: function (filters, aspect) {
            ...
            filters.forEach(function (item) {
                if (usedFiltersArray.indexOf(item.filterLabel) < 0) usedFiltersArray.push(item.filterLabel);
            });

            filters.forEach(function (item) {
                if (!aspectsMap[item.aspectLabel]) aspectsMap[item.aspectLabel] = {};
                let key = item.class + "|" + item.property;
                if (!aspectsMap[item.aspectLabel][key]) aspectsMap[item.aspectLabel][key] = [];
                let itemFilters = [];
