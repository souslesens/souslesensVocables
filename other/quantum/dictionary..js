...
async.eachSeries(files, function(filePath,callbackEach){
    var readStream = fs.createReadStream(filePath);
    var writeStream = fs.createWriteStream(outputPath, {flags: 'a'});
    readStream.pipe(writeStream);

    readStream.on('error', function(err){console.log(err);writeStream.end()});
    readStream.on('end', function(err)
    {callbackEach();})
    writeStream.on('error',  function(err){console.log(err);writeStream.end()})
