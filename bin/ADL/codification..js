...
var processTEPDKtags = function () {
    var str = "";
    str += "type\titem.TagNumber\t functionCode\tfunctionLabel\tmoduleCode\tmoduleLabel\tlocationCode\tlocationLabel\tADL.FunctionalClassID\tADL.FunctionalClassLabel\tADL.ServiceDescription\n"
    var GormCodesMap = {}
    var codesMap = {}
    var count = 0
            ...
            for (var key in codeRegExTEPDK) {
             ...
             //   var array = regex.exec(item.TagNumber);
                ...
                if (array) {
                    ...
                    var moduleLabel = ""
                    if (moduleCode && modulesMap[moduleCode])
                        moduleLabel = modulesMap[moduleCode];
                   ...
                   // str += key + "\t" + item.TagNumber + "\t" + functionCode + "\t" + functionLabel + "\t" + moduleCode + "\t" + moduleLabel + "\t" + locationCode + "\t" + locationLabel + "\t" + item.FunctionalClassID + "\t" + item.FunctionalClassLabel + "\t" + item.ServiceDescription + "\n"
                    str += key + "\t" + item.tagName + "\t" + functionCode + "\t" + functionLabel + "\t" + moduleCode + "\t" + moduleLabel + "\t" + locationCode + "\t" + locationLabel + "\t" + item.Functional_Location_code + "\t" + item.Manufacturer + "\t" + item.Description + "\n"
    ...
    async.series([
        ...
        function (callbackSeries) {
            ...
            query=" select * FROM [TEPDK].[dbo].[functional_location] " +
