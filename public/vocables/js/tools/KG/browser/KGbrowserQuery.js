var KGbrowserQuery = (function () {
    ...
    self.onQueryParamsDialogValidate = function (logicalMode) {
        ...
        if (self.queryMode == "expandGraphNode") {
            ...
            var setFilterCountUI = function () {
                ...
                var iconUrl = null;
                if (KGbrowserCustom.superClassesMap[nodeData.class]) iconUrl = null; //KGbrowserCustom.iconsDir + KGbrowserCustom.superClassesMap[nodeData.class].group.toLowerCase() + ".png";
                ...
                var html =
                    ...
                    filterId +
                    "\")'>X</button>&nbsp;";
                if (iconUrl) html += "<img src='" + iconUrl + "' width='25'/>";
                html += "<span style='font-weight:normal;color:" + "black" + "'>" + superClassLabel + " : " + nodeData.count + "<br><i> " + filterLabel + "</i></span>";
                ("</div>");
    ...
    self.executeShortestPathQuery = function (fromNode, toNode, options, callback) {
        ...
        if (self.queryFilterNodes.length == 0) return alert("create a filter first");
    ...
    self.initClassesTab = function (classes, model) {
        ...
        classesArray.forEach(function (classId) {
            ...
            if (!distinctNode[id1]) {
                distinctNode[id1] = 1;
                var jstreeType = null;
                if (KGbrowserCustom.superClassesMap[classId]) jstreeType = KGbrowserCustom.superClassesMap[classId].group;
                jstreeData.push({
                    id: id1,
                    text: label,
                    parent: "#",
                ...
                if (model[predicate]) {
                    ...
                    classes[classId][predicate].forEach(function (object) {
                        ...
                        if (!distinctNode[id3]) {
                            distinctNode[id3] = 1;
                            var jstreeType = null;
                            if (KGbrowserCustom.superClassesMap[object]) jstreeType = KGbrowserCustom.superClassesMap[object].group;
                            jstreeData.push({
                                id: id3,
