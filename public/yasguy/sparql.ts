...
export function getAjaxConfig(yasqe: Yasqe, _config: Partial<Config["requestConfig"]> = {}): PopulatedAjaxConfig | undefined {
    const config: RequestConfig<Yasqe> = merge({}, getRequestConfigSettings(yasqe, yasqe.config.requestConfig), getRequestConfigSettings(yasqe, _config));
    ...
    var reqMethod: "GET" | "POST" = queryMode == "update" ? "POST" : isFunction(config.method) ? config.method(yasqe) : config.method;
...
export function getAcceptHeader(yasqe: Yasqe, _config: Config["requestConfig"]) {
    ...
    if (yasqe.getQueryMode() == "update") {
        ...
        if (qType == "DESCRIBE" || qType == "CONSTRUCT") {
            ...
            acceptHeader = isFunction(config.acceptHeaderSelect) ? config.acceptHeaderSelect(yasqe) : config.acceptHeaderSelect;
        }
    }
    return acceptHeader;
}
