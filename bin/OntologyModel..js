...
var OntologyModel={
  ...
  if(sourceObj)
    serverUrl=sourceObj.sparql_server.url;
  else
    serverUrl=OntologyModel.Config.default_sparql_url
  ...
  getOntologyModel:function(source,graphUri,callback){
    if(OntologyModel.instances[source])
      return callback(null,OntologyModel.instances[source])
            ...
            function (callbackSeries) {
             ...
             // Sparql_proxy.querySPARQL_GET_proxy(url, query, null, {}, function (err, result) {
                ...
                result.results.bindings.forEach(function (item) {
                  ...
                  ontologyModel.constraints[item.prop.value].domainLabel = item.domainLabel
                    ? item.domainLabel.value
                    : OntologyModel.util.getLabelFromURI(item.domain.value);
                });
                callbackSeries();
              });
            },
            ...
            function (callbackSeries) {
              ...
              //  Sparql_proxy.querySPARQL_GET_proxy(url, query, null, {}, function (err, result) {
                ...
                result.results.bindings.forEach(function (item) {
                  ...
                  ontologyModel.constraints[item.prop.value].rangeLabel = item.rangeLabel
                    ? item.rangeLabel.value
                    : OntologyModel.util.getLabelFromURI(item.range.value);
                });
                callbackSeries();
              });
            },
...
Sparql_OWL.getDictionary(source, { lang: Config.default_lang, filter: filter }, null, function (err, result) {
 ...
 util:{
    getVariableLangLabel :function (variable, optional, skosPrefLabel) {
     ...
     var str = "?" + variable + " rdfs:label" + pred + " ?" + variable + "Label. filter(regex( lang(?" + variable + "Label), '" + OntologyModel.Config.default_lang + "') || !lang(?" + variable + "Label))";
