...
if (true) {
        ...
        query: {
            bool: {
                must: [
                    {
                        terms: {
                            "Concepts.Basin.instances.keyword": ["Amadeus Basin", "Amadeus Basins"],
                        },
                    },
                ],
                should: [
                    {
                        terms: {
                            "Concepts.Fluid.instances.keyword": ["Fluid Oil", "Fluid Water"],
                        },
                    },
                ],
        ...
        query: {
            bool: {
                must: [
                    {
                        term: {
                            "Concepts.Log.name.keyword": ["Adavale Basin", "Adriatic Basin", "African basins"],
                        },
                    },
                ],
            },
        ...
        query: {
            bool: {
                must: [
                    {
                        terms: {
                            "Concepts.Basin.instances.keyword": ["Abu Attifel basin", "Abu Dhabi Basin", "Abu Gharadig Basin"],
                        },
                    },
                ],
                filter: [
    ...
    executeGaiaQuery(query, indexes, {}, function (err, result) {
        result.hits.hits.forEach(function (hit) {
            if (!hit._source.Concepts.Fluid) var x = 3;
        });
