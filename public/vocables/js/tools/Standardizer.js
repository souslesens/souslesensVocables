var Standardizer = (function() {
  ...
  self.addDictionaryMatches = function(sourceObjects, callback) {
    ...
    Lineage_dictionary.queryTSFdictionary(Config.dictionarySource, filter, function(err, result) {
      ...
      result.forEach(function(item) {
        if (!dictionaryMatches[item.domain.value])
          dictionaryMatches[item.domain.value] = [];
        dictionaryMatches[item.domain.value].push(item);
      });

      sourceObjects.forEach(function(item) {
        if (dictionaryMatches[item.id])
          item.dictionary = dictionaryMatches[item.id];
      });
  ...
  self.editCellData = function(cellData) {
    ...
    if (cellData.sourceObject)
      cellData = cellData.sourceObject;
    ...
    if(cellData.dictionary){
      cellData.dictionary.forEach(function(item){
        html += "<tr><td>" +"TSF_dictionary" + "</td><td>" + "<span class='Standardizer_dictionaryEntry' onclick='Standardizer.showDictionaryEntry(\""+item.node.value+"\")'>"+item.domainSourceLabel.value+"->"+item.rangeSourceLabel.value + "</td></tr>";
      })
    ...
    async.eachSeries(
      ...
      function(words, callbackEach) {
        SearchUtil.getElasticSearchMatches(words, indexes, searchType, 0, 10000, function(err, result) {
          if (err) return alert(err);
          var entities = [];
          if (!result.forEach || result.length == 0) return MainController.UI.message("no result)");
          result.forEach(function(item) {
