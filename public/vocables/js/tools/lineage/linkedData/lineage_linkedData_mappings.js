...
https: var Lineage_linkedData_mappings = (function() {
  ...
  self.getSourceJoinsMappings = function(source, options, callback) {
    ...
    async.series([
      function(callbackSeries) {
        ...
        Sparql_OWL.getObjectRestrictions(source, null, { filter: filter }, function(err, result) {
          ...
          result.forEach(function(item) {
            if( joinsIds.indexOf(item.join.value)<0)
              joinsIds.push(item.join.value);
            joinsMap[item.join.value] = {
              bNode: item.node.value,
              from: {classId:item.concept.value,
                classLabel:item.conceptLabel.value
              },
              to: {classId:item.value.value,
                classLabel:item.valueLabel.value
              },
      ...
      var filterStr=Sparql_common.setFilter("join",joinsIds)
        var query = "" +
          "" +
          "select * FROM  <" +Config.linkedData_mappings_graph + "> where {" +
          "?join ?p ?o. "+filterStr+
        "} ";
  ...
  self.clearSourceMappings = function(source) {
    ...
    if (!confirm(" delete mappings in  source" + source))
      return;
    ...
    async.series([
      ...
      function(callbackSeries) {
        var query = "select * " +
          "where {?s ?p ?o filter( ?p in (<" + Config.linkedData_mappings_graph + "hasColumnMapping>,<" + Config.linkedData_mappings_graph + "hasSqlJoin>))}";
        Sparql_proxy.querySPARQL_GET_proxy(url, query, "", { source: source }, function(err, result) {
          if (err) {
            return callbackSeries(err);
          }
