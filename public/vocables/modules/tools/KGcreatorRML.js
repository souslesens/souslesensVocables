...
var KGcreator_RML = (function () {
    ...
    self.onLoaded = function () {
        $("#actionDivContolPanelDiv").load("snippets/KGcreator_RML/leftPanel.html", function () {
            self.loadCsvDirs();
            self.showSourcesDialog(function (err, result) {
                if (err) {
                    return alert(err.responseText);
                }
                $("#graphDiv").load("snippets/KGcreator_RMLRML/centralPanel.html", function () {
                    self.initCentralPanel();
                });
                $("#rightPanelDiv").load("snippets/KGcreator_RML/rightPanel.html", function () {
                    // pass
                });
            });
        });
    ...
    self.initSource = function (source, callback) {
        ...
        $("#KGcreator_RML_owlSourceInput").html(source);

        $("#KGcreator_RML_topLevelOntologiesInput").html(Config.currentTopLevelOntology);
    ...
    self.loadDataBases = function (callback) {
        ...
        $.ajax({
            ...
            success: function (data, _textStatus, _jqXHR) {
                common.fillSelectOptions("KGcreator_RML_csvDirsSelect", data, true, "name", "name");
                if (callback) {
                    return callback();
                }
            },
    ...
    self.onTopLevelOntologyChange = function (topLevelOntology) {
        // Config.currentTopLevelOntology = $("#KGcreator_RML_topLevelOntologiesSelect").val();
        self.initCentralPanel();
    };
    ...
    self.loadCsvDirs = function (options) {
        ...
        $.ajax({
            ...
            success: function (result, _textStatus, _jqXHR) {
                common.fillSelectOptions("KGcreator_RML_csvDirsSelect", result, true);
            },
            error: function (err) {
    ...
    self.listTables = function (db, callback) {
        self.currentDbName = db ? db : $("#KGcreator_RML_csvDirsSelect").val();
    ...
    self.listFiles = function (currentCsvDir = null) {
        self.currentCsvDir = currentCsvDir ? currentCsvDir : $("#KGcreator_RML_csvDirsSelect").val();
        var payload = {
            dir: "CSV/" + self.currentCsvDir,
        };
        $.ajax({
    ...
    self.showTablesTree = function (data) {
        self.loadMappingsList(function (err) {
            ...
            var options = {
                openAll: true,
                selectTreeNodeFn: KGcreator_RML.onCsvtreeNodeClicked,
                contextMenu: KGcreator_RML.getContextMenu(),
                //  withCheckboxes: true,
            };
            data.forEach(function (file) {
                ...
                for (var key in self.mappingFiles) {
                    if (key.indexOf(file) > -1) {
                        label = "<span class='KGcreator_RML_fileWithMappings'>" + file + "</span>";
                    }
                }
            ...
            JstreeWidget.loadJsTree("KGcreator_RML_csvTreeDiv", jstreeData, options);
        });
    };
    ...
    self.getSystemsTreeContextMenu = function () {
        ...
        items.addBasicIndividual = {
            ...
            action: function (_e) {
                // pb avec source
                KGcreator_RML.addBasicMapping("owl:NamedIndividual");
            },
        };
        items.addBasicClass = {
            label: "addBasicClass",
            action: function (_e) {
                // pb avec source
                KGcreator_RML.addBasicMapping("owl:Class");
            },
        };
        items.addBasicBag = {
            label: "addBasicBag",
            action: function (_e) {
                // pb avec source
                KGcreator_RML.addBasicMapping("rdf:Bag");
            },
        };
        ...
        items.setAsSubject = {
            ...
            action: function (_e) {
                ...
                $("#KGcreator_RML_subjectInput").val(self.currentTreeNode.data.id);
            },
        };
        ...
        items.showLookupDialog = {
            ...
            action: function (_e) {
                ...
                $("#KGcreator_RML_dialogDiv").load("snippets/KGcreator_RML/lookupDialog.html", function () {
                    $("#KGcreator_RML_lookupFileName").val(self.currentTreeNode.data.id);
                });
                $("#KGcreator_RML_dialogDiv").dialog("open");
            },
        };
        items.showTransformDialog = {
            ...
            action: function (_e) {
                ...
                $("#KGcreator_RML_dialogDiv").load("snippets/KGcreator_RML/transformDialog.html", function () {
                    $("#KGcreator_RML_transformColumn").val(self.currentTreeNode.data.id);
                });
                $("#KGcreator_RML_dialogDiv").dialog("open");
            },
        };
    ...
    self.editFile = function () {
        ...
        $.ajax({
            ...
            success: function (_result, _textStatus, _jqXHR) {
                $("#KGcreator_RML_saveFileButton").css("display", "block");
                $("#KGcreator_RML_dataSampleDiv").val(_result);
            },
            error(err) {
                if (callback) {
                    return callback(err);
    ...
    self.saveFile = function () {
        ...
        $("#KGcreator_RML_saveFileButton").css("display", "none");
        var str = $("#KGcreator_RML_dataSampleDiv").val();
        ...
        $.ajax({
            ...
            success: function (_result, _textStatus, _jqXHR) {
                ...
                $("#KGcreator_RML_dataSampleDiv").val("");
            },
            error(err) {
                return alert(err.responseText);
            },
        ...
        async.series(
            [
                ...
                function (callbackSeries) {
                    ...
                    common.fillSelectOptions("KGcreator_RML_subjectSelect", currentSubjectClasses, true, "label", "id");
                    return callbackSeries();
                },
                function (callbackSeries) {
                    $("#editPredicate_mainDiv").remove()
                    $("#sharedPredicatesPanel").load("snippets/commonUIwidgets/editPredicateDialog.html", function () {
                        PredicatesSelectorWidget.init(KGcreator_RML.currentSlsvSource, function () {
                            PredicatesSelectorWidget.onSelectObjectFn = function (value) {};
                            PredicatesSelectorWidget.onSelectPropertyFn = function (value) {};
                        });

                        $("#editPredicate_customPredicateContentDiv").html("<div> <input type='checkbox' id='KGcreator_RML_isRestrictionCBX' />is Restriction</div>");
                        var html =
                            ' is String<input type="checkbox" id="KGcreator_RML_isObjectStringCBX" /> ' +
                            "lookup <input id=\"KGcreator_RML_objectLookupName\" style='width:100px'/>" +
                            ' <button onclick="KGcreator_RML.handleAddClick()">Add</button></div>';
                ...
                function (callbackSeries) {
                    return callbackSeries();
                    self.fillPredicatesSelect(self.currentSlsvSource, "KGcreator_RML_predicateSelect", { usualProperties: true }, function (err) {
                ...
                function (callbackSeries) {
                    ...
                    Lineage_upperOntologies.getTopOntologyClasses(Config.currentTopLevelOntology, {}, function (err, result) {
                        ...
                        common.fillSelectOptions("KGcreator_RML_usualObjectsSelect", usualObjectClasses, true, "label", "id");
                ...
                function (callbackSeries) {
                    self.mainJsonEditor = new JsonEditor("#KGcreator_RML_mainJsonDisplay", {});
                    $("#KGcreator_RML_mainJsonDisplay").on("click", () => {
                        self.mainJsonEditorModified = true;
                    });

                    $("#KGcreator_RML_dialogDiv").dialog({
                        autoOpen: false,
                        height: 600,
                        width: 600,
                        modal: false,
    ...
    self.addBasicMapping = function (type) {
        ...
        $("#KGcreator_RML_subjectInput").val(column);
    };
        ...
        if (self.currentSourceType == "CSV") {
            ...
            $.ajax({
                ...
                success: function (result, _textStatus, _jqXHR) {
                    ...
                    JstreeWidget.addNodesToJstree("KGcreator_RML_csvTreeDiv", obj.node.id, jstreeData);
    ...
    self.generateRML = function () {
        ...
        $("#KGcreator_RML_tripleMessageDiv").html("");
        var subject = $("#KGcreator_RML_subjectInput").val();

        var predicate = $("#editPredicate_propertyValue").val();
        var object = $("#editPredicate_objectValue").val();

        var isObjectString = $("#KGcreator_RML_isObjectStringCBX").prop("checked");

        var subjectLookupName = $("#KGcreator_RML_subjectLookupName").val();
        var objectLookupName = $("#KGcreator_RML_objectLookupName").val();
        var isRestrictionCBX = $("#KGcreator_RML_isRestrictionCBX").prop("checked");
        var isSpecificPredicate = $("#KGcreator_RML_isSpecificPredicateCBX").prop("checked");

        var TripleObject = $("#subjectSelect").val();

        $("#KGcreator_RML_objectSelect").val("");
        $("#KGcreator_RML_predicateSelect").val("");
    ...
    function showModal() {
        ...
        modalButton.onclick = KGcreator_RML.generateRML;
    }
    ...
    self.createRDFTriples = function () {
        ...
        // Read the RML file
        var rmlFile = new File([], "C:/Users/user/Desktop/KGcreator_RML/mapping.rml", { type: "text/plain" });
        var rmlReader = new FileReader();
        rmlReader.onload = function (event) {
            rmlFile = new File([event.target.result], "C:/Users/user/Desktop/KGcreator_RML/mapping.rml", { type: "text/plain" });
            formData.append("rml", rmlFile);

            // Read the data file
            var dataFile = new File([], "C:/Users/user/Desktop/KGcreator_RML/data.csv", { type: "text/csv" });
            var dataReader = new FileReader();
            dataReader.onload = function (event) {
                dataFile = new File([event.target.result], "C:/Users/user/Desktop/KGcreator_RML/data.csv", { type: "text/csv" });
    ...
    self.addTripleToTA = function () {
        $("#KGcreator_RML_tripleMessageDiv").html("");
        var subject = $("#KGcreator_RML_subjectInput").val();
        //  var predicate = $("#KGcreator_RML_predicateInput").val();
        //  var object = $("#KGcreator_RML_objectInput").val();
...
predicate = self.getPredefinedPart14PredicateFromClasses(subject, object);
        ...
        $("#KGcreator_RML_objectInput").val("");
        $("#KGcreator_RML_objectSelect").val("");
        $("#KGcreator_RML_predicateInput").val("");
        $("#KGcreator_RML_predicateSelect").val("");

        //   $("#KGcreator_RML_tripleTA").val(JSON.stringify(tripleObj))
    };
    ...
    self.onTripleModelSelect = function (role, value) {
        ...
        if (role == "s") {
            if (value == "_selectedColumn") {
                $("#KGcreator_RML_subjectInput").val(self.currentTreeNode.text);
            } else {
                $("#KGcreator_RML_subjectInput").val(value);
            }
            //  $("#KGcreator_RML_subjectSelect").val("");
        } else if (role == "p") {
            $("#KGcreator_RML_predicateInput").val(value);

            //   $("#KGcreator_RML_predicateSelect").val("");
        } else if (role == "o") {
            if (value == "_selectedColumn") {
                $("#KGcreator_RML_objectInput").val(self.currentTreeNode.text);
            } else {
                $("#KGcreator_RML_objectInput").val(value);
            }
    ...
    self.addLookup = function () {
        var lookup = {
            name: $("#KGcreator_RML_lookupName").val(),
            fileName: $("#KGcreator_RML_lookupFileName").val(),
            sourceColumn: $("#KGcreator_RML_lookupSourceColumn").val(),
            targetColumn: $("#KGcreator_RML_lookupTargetColumn").val(),
            transformFn: $("#KGcreator_RML_lookupTransformFn").val().replace(/"/g, "'"),
        };
        self.currentJsonObject = self.mainJsonEditor.get();
        self.currentJsonObject.lookups.push(lookup);
        self.mainJsonEditor.load(self.currentJsonObject);
        self.mainJsonEditorModified = true;
        $("#KGcreator_RML_dialogDiv").dialog("close");
    };

    self.showFunctionDialog = function (_role) {
        $("#KGcreator_RML_dialogDiv").load("snippets/KGcreator_RML/functionDialog.html");
        $("#KGcreator_RML_dialogDiv").dialog("open");
    };
    self.testFunction = function () {
        var fnBody = $("#KGcreator_RML_fnBody").val();
        fnBody = fnBody.replace(/"/g, "'");
        try {
            new Function("row", "mapping", fnBody);
            $("#KGcreator_RML_testFnResult").html("function OK");
        } catch (err) {
            $("#KGcreator_RML_testFnResult").html("error in function code " + err.message);
        }
    };
    self.addFunction = function () {
        ...
        if (role == "s") {
            $("#KGcreator_RML_subjectInput").val(fnObject);
        } else if (role == "p") {
            $("#KGcreator_RML_predicateInput").val(fnObject);
        } else if (role == "o") {
            $("#KGcreator_RML_objectInput").val(fnObject);
        }
        $("#KGcreator_RML_dialogDiv").dialog("close");
    };

    self.addTransformFunction = function () {
        var column = $("#KGcreator_RML_transformColumn").val();
        var fnBody = $("#KGcreator_RML_fnBody").val();
        fnBody = fnBody.replace(/"/g, "'");
...
self.saveMappings({classId:classId})
    ...
    self.saveMappings = function (options, callback) {
        ...
        self.checkModel(self.currentSource, function (err, result) {
            if (err) {
                $("#KGcreator_RML_dataSampleDiv").val(err);
    ...
    self.setUpperOntologyPrefix = function () {
        ...
        if (!self.currentJsonObject.prefixes) {
            ...
            $("#KGcreator_RML_topLevelOntologiesSelect").val(currentTopLevelOntology);
        }
    };
    ...
    self.createTriples = function (test, _options) {
        ...
        $("#KGcreator_RML_dataSampleDiv").val("creating triples...");
        ...
        if ($("#KGcreator_RML_onlySelectionCBX").prop("checked")) {
            ...
            $.ajax({
                ...
                success: function (result, _textStatus, _jqXHR) {
                    if (test) {
                        ...
                        $("#KGcreator_RML_dataSampleDiv").val(str);
                        MainController.UI.message("", true);
                    } else {
                        $("#KGcreator_RML_dataSampleDiv").val(result.countCreatedTriples + " triples created in graph " + self.currentJsonObject.graphUri);
    ...
    self.indexGraph = function () {
        ...
        if (confirm("index source " + graphSource)) {
            SearchUtil.generateElasticIndex(graphSource, null, function (err, _result) {
                ...
                $("#KGcreator_RML_dataSampleDiv").val("indexed graph " + self.currentJsonObject.graphUri + " in index " + graphSource.toLowerCase());
            });
        }
    };
    ...
    self.deleteKGcreator_RMLTriples = function (deleteAllGraph) {
        if (!self.currentJsonObject) {
            return;
        } //alert("no file mappings selected");
        if (!self.currentJsonObject.graphUri) {
        ...
        if (deleteAllGraph) {
            if (!confirm("Do you really want to delete  triples created with KGcreator_RML in " + self.currentJsonObject.graphUri)) {
                return;
            }

            var filter = "?p =<http://purl.org/dc/terms/creator> && ?o='KGcreator_RML'";
            ...
            if (!confirm("Do you really want to delete  triples created with KGcreator_RML in " + self.currentJsonObject.fileName)) {
                return;
            }
    ...
    self.graphMappings = function () {
        ...
        var html = "<div id='KGcreator_RML_mappingsGraphDiv' style='width:900px;height:750px'></div>";
        $("#mainDialogDiv").dialog("open");
        $("#mainDialogDiv").html(html);
        visjsGraph.draw("KGcreator_RML_mappingsGraphDiv", visjsData, {}, function () {});
    };
        ...
        Sparql_OWL.getObjectProperties(source, { withoutImports: 1 }, function (err, result) {
            ...
            Lineage_upperOntologies.getUpperOntologyObjectPropertiesDescription(Config.currentTopLevelOntology, false, function (err, result) {
                ...
                if (options.usualProperties) {
                    KGcreator_RML.usualProperties.forEach(function (item) {
...
export default KGcreator_RML;

window.KGcreator_RML = KGcreator_RML;
