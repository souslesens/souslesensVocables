var Lineage_containers = (function() {
  ...
  self.search = function() {
    ...
    var filter = "";
    if (term)
      filter = Sparql_common.setFilter("container", null, term);
  ...
  self.addContainer = function(source) {
    ...
    var containerUri = Config.sources [source].graphUri + common.formatStringForTriple(newContainerLabel, true);
    ...
    Sparql_generic.insertTriples(source, triples, null, function(err, result) {
      ...
      if (!$("#lineage_containers_containersJstree").jstree) {// initialize jstree
        self.search(function(err, result) {
          $("#lineage_containers_containersJstree").jstree().create_node(parent, newNode, "first", function(err, result) {
...
if(container.type!="container")
  ...
  self.onMoveContainer = function(data, element, helper, event) {
    ...
    var query = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>" +
      "with <" + graphUri + ">" +
      "delete {?s rdfs:member <" + sourceNodeId + ">}" +
      "insert {<" + targetNodeId + "> rdfs:member <" + sourceNodeId + ">}";
